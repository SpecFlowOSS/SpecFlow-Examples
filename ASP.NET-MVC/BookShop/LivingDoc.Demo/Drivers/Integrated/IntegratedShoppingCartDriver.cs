...
using LivingDoc.Demo.Support.Webserver;
...
namespace LivingDoc.Demo.Drivers.Integrated
{
    public class IntegratedShoppingCartDriver : IShoppingCartDriver
    {
        ...
        public void ContainsTypesOfItems(int expectedAmount)
        {
            ...
            actionResult.Model<BookShop.Mvc.Models.ShoppingCart>().Lines.Should().HaveCount(expectedAmount);
        }
        ...
        public void ContainsTotalItems(int expectedQuantity)
        {
            using var controller = GetShoppingCartController();
            var actionResult = controller.Index();
            actionResult.Model<BookShop.Mvc.Models.ShoppingCart>().Count.Should().Be(expectedQuantity);
        }
        ...
        public void ShowsTotalPriceOf(decimal expectedTotalPrice)
        {
            using var controller = GetShoppingCartController();
            var actionResult = controller.Index();
            actionResult.Model<BookShop.Mvc.Models.ShoppingCart>().Price.Should().Be(expectedTotalPrice);
        }
        ...
        public void ContainsCopiesOf(string bookTitle, int expectedQuantity)
        {
            ...
            using var controller = GetShoppingCartController();
            var actionResult = controller.Index();
            actionResult.Model<BookShop.Mvc.Models.ShoppingCart>().Lines
